...
export (Array, PackedScene) var fruit_intance
...
func create_fruit():
	var fruit_number = round(rand_range(0, fruit_intance.size() -1))
	var fruit = fruit_intance[fruit_number].instance()
	fruit.global_position = $spawnPoint.global_position
	fruit.apply_impulse(Vector2.ZERO, Vector2(rand_range(30, -30), -90))
